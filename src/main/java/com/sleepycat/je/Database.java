...
public class Database implements Closeable {
    ...
    /**
     * Create a ForwardCursor to iterate over the records in 'this'.  Because
     * the retrieval is based on Log Sequence Number (LSN) order rather than
     * key order, records are returned in unsorted order in exchange for
     * generally faster retrieval.  LSN order approximates disk sector order.
     *
     * The records returned by the scan correspond to the state of the data at
     * the beginning of the scan plus some, but not all, changes made by the
     * application after the start of the scan.  The user should not rely on
     * the scan returning any changes made after the start of the scan.
     * Specifically, if the record referred to by the ForwardCursor in a
     * DiskOrderedScan is deleted after the ForwardCursor is positioned at that
     * record, getCurrent() will still return the key and value of that record
     * and OperationStatus.SUCCESS.
     *
     * As with a READ_UNCOMMITTED scan, changes made by all transactions,
     * including uncommitted transactions, may be returned by the scan.  Also,
     * a record returned by the scan is not locked, and may be modified or
     * deleted by the application after it is returned, including modification
     ...
     *
     * <p><em>WARNING:</em> After calling this method, deletion of log files by
     * the JE log cleaner will be disabled until {@link ForwardCursor#close()}
     * is called.  To prevent unbounded growth of disk usage, be sure to call
     * {@link ForwardCursor#close()} to re-enable log file deletion.</p>
     *
     * <p><em>WARNING:</em> During initialization of a Disk Ordered Scan,
     * the internal nodes of the btree are latched for read and this may cause
     * reduced performance for concurrent update operations.  This pause
     * may be limited using
     * {@link DiskOrderedCursorConfig#setMaxSeedMillisecs} or
     * {@link DiskOrderedCursorConfig#setMaxSeedNodes}.
     */
